...
use Idoheo\StringGenerator\StringGenerator\NqsCharStringGenerator;
use Idoheo\Tests\StringGenerator\TestCase;

/**
 * @coversDefaultClass \Idoheo\StringGenerator\StringGenerator\NqsCharStringGenerator
 */
class NqsCharStringGeneratorTest extends TestCase
{
    /**
    ...
    {
        parent::setUp();
        $this->stringGenerator = new NqsCharStringGenerator();
    }
    ...
    public function testGenerateString()
    {
        for ($i = 1; $i <= 5; ++$i) {
            ...
            static::assertRegExp(
                \sprintf('/^[\x{20}-\x{21}\x{23}-\x{5B}\x{5D}-\x{7E}]{%d}$/u', $length),
